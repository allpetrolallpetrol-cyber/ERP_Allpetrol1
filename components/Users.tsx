...
import { Save, UserPlus, Search, Edit2, Shield, Briefcase, Mail, X, CheckSquare, Lock, Image as ImageIcon, Link as LinkIcon, Upload, AlertCircle, User as UserIcon, Key } from 'lucide-react';
...
const UserModal = ({ user, onClose, onSave }: { user: Partial<User> | null, onClose: () => void, onSave: (u: User) => void }) => {
    ...
    const cleanGoogleUrl = (url: string) => {
        if (!url) return '';
        if (url.includes('google.com/url') || url.includes('google.com.ar/url')) {
    ...
    const handleAvatarUrlChange = (val: string) => {
        const clean = cleanGoogleUrl(val);
        setImgError(false);
        setFormData(prev => ({ ...prev, avatarUrl: clean }));
    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (file.size > 500 * 1024) {
                showToast("La imagen es demasiado grande. Máximo 500KB.", 'error');
                return;
            }
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[95vh]">
                ...
                <div className="flex border-b border-zinc-200">
                    <button 
                        ...
                        className={`flex-1 py-3 text-sm font-medium text-center transition-colors ${activeTab === 'GENERAL' ? 'text-accent border-b-2 border-accent bg-zinc-50' : 'text-zinc-500 hover:text-zinc-700'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-3 text-sm font-medium text-center transition-colors ${activeTab === 'PERMISSIONS' ? 'text-accent border-b-2 border-accent bg-zinc-50' : 'text-zinc-500 hover:text-zinc-700'}`}
                    >
                        <div className="flex items-center justify-center">
                            <Key size={16} className="mr-2"/> Permisos y Accesos
                        </div>
                    </button>
                </div>
                            ...
                            <div className="animate-in slide-in-from-left-4 fade-in duration-300 space-y-6">
                                ...
                                <div className="flex items-start gap-6">
                                    <div className="shrink-0 relative group">
                                        <div className="w-24 h-24 rounded-full bg-zinc-200 border-4 border-white shadow-lg overflow-hidden flex items-center justify-center relative">
                                        ...
                                        {formData.avatarUrl && (
                                            <button 
                                                ...
                                                className="absolute -top-1 -right-1 bg-red-500 text-white p-1 rounded-full hover:bg-red-600 shadow-sm z-10"
                                                title="Eliminar foto"
                                            >
                                                <X size={12}/>
                                            </button>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-zinc-500 mb-1">Legajo</label>
                                        <input 
                                            type="text" 
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-zinc-500 mb-1">Cargo / Rol Sistema</label>
                                        <div className="relative">
                                            <select 
                                                className="w-full pl-9 pr-3 py-2 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-accent outline-none bg-white appearance-none"
                                                value={formData.role}
                                ...
                                <div className="bg-purple-50 border border-purple-100 p-4 rounded-xl flex items-center justify-between">
                                    <div>
                                        <h4 className="text-sm font-bold text-purple-800 flex items-center">
                                            <CheckSquare size={16} className="mr-2"/> Usuario Aprobador
                                        </h4>
                                        <p className="text-xs text-purple-600 mt-1">
                                            Habilitar este usuario para figurar en esquemas de liberación de compras.
                                        </p>
                                    </div>
                            ...
                            <div className="animate-in slide-in-from-right-4 fade-in duration-300">
                                <div className="bg-zinc-50 p-5 rounded-xl border border-zinc-200">
                                    <h4 className="text-sm font-bold text-zinc-700 mb-1 flex items-center">
                                        <Lock size={16} className="mr-2"/> Matriz de Permisos (Granular)
                                    </h4>
                                    <p className="text-xs text-zinc-400 mb-4">
                                        Asigne niveles de acceso específicos para cada funcionalidad.
                                    </p>
                                    
                                    <div className="space-y-6">
                                        {Object.entries(groupedModules).map(([category, modules]) => (
                                            <div key={category} className="bg-white rounded-lg border border-zinc-200 overflow-hidden shadow-sm">
                                                <div className="bg-zinc-100 px-4 py-2 border-b border-zinc-200 font-bold text-xs text-zinc-600 uppercase flex items-center">
                                                    {category}
                                                </div>
                                                <div className="divide-y divide-zinc-100">
                                                    {(modules as any[]).map(module => {
                                                        ...
                                                        return (
                                                            <div key={module.id} className="p-3 flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-zinc-50 transition-colors">
                                                                <div className="flex-1">
                                                                ...
                                                                <div className="flex-shrink-0 w-full md:w-40">
                                                                    <select 
                                                                        className={`w-full px-2 py-1.5 rounded border text-xs font-semibold outline-none focus:ring-1 focus:ring-accent cursor-pointer ${
                                                                            currentLevel === 'NONE' ? 'bg-zinc-50 text-zinc-400 border-zinc-200' : 
                                                                            currentLevel === 'ADMIN' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                                                                            currentLevel === 'CREATE' ? 'bg-green-50 text-green-700 border-green-200' :
                                                                            currentLevel === 'EDIT' ? 'bg-orange-50 text-orange-700 border-orange-200' :
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 text-sm">Gestión de personal, roles y matriz de permisos por funcionalidad.</p>
                </div>
                <button 
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-zinc-100">
                            {filteredUsers.map(user => {
                                ...
                                const isAdmin = permissions['USERS_ADMIN'] === 'ADMIN' || user.role === 'ADMIN';

                                return (
                                    <tr key={user.id} className="hover:bg-zinc-50 transition-colors group">
                                        <td className="px-6 py-3">
                                            <div className="w-8 h-8 rounded-full bg-zinc-200 border border-zinc-300 flex items-center justify-center overflow-hidden relative">
                                                {user.avatarUrl ? (
                                                    <img 
                                                        src={user.avatarUrl} 
                                                        alt="" 
                                                ...
                                                <span className={`text-xs font-bold text-zinc-400 ${user.avatarUrl ? 'hidden' : ''}`}>
                                                    {user.firstName[0]}{user.lastName[0]}
                                                </span>
                                                {user.isApprover && (
                                                    <div className="absolute -bottom-1 -right-1 bg-purple-500 text-white rounded-full w-4 h-4 flex items-center justify-center border border-white" title="Usuario Aprobador">
                                                        <CheckSquare size={10} />
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <td className="px-6 py-3">
                                            <div className="flex flex-wrap gap-1">
                                                ...
                                                {!isAdmin && activeModulesCount > 0 && (
                                                    <span className="text-zinc-600 text-xs">{activeModulesCount} func. habilitadas</span>
                                                )}
                                            </div>
                                        </td>

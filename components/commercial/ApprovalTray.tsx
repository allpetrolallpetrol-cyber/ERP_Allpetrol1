                    ...
                    <div key={rfq.id} className="bg-white p-6 rounded-xl border-l-4 border-orange-50 shadow-sm relative overflow-hidden group">
                        ...
                        <div className="flex justify-between items-end mb-4">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-[10px] text-slate-400">MONTO NETO</p>
                            </div>
                        </div>
                        
                        <div className="bg-slate-50 p-3 rounded-lg text-sm text-slate-600 mb-4 max-h-32 overflow-y-auto border border-slate-100">
                            <ul className="space-y-1">
                                {rfq.items.map((item, idx) => (
                                    <li key={idx} className="flex justify-between items-center">
                                        <span>• {item.description}</span>
                                        <span className="font-mono text-xs font-bold text-slate-400">x{item.quantity}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>

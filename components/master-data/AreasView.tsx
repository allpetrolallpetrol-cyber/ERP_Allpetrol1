    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-full flex flex-col animate-in fade-in">
            ...
            <div className="flex gap-4 mb-6">
                ...
                />
                <button onClick={handleAdd} className="bg-slate-900 text-white px-6 rounded-lg font-bold hover:bg-slate-800 transition-colors">
                    Agregar
                </button>
            </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 border-b font-semibold text-slate-600">
                        <tr>
                            ...
                            <th className="p-4 text-right">ID Sistema</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {areas.map(area => (
                            <tr key={area.id} className="hover:bg-slate-50 transition-colors">
                                <td className="p-4 font-medium text-slate-700">{area.name}</td>
                                <td className="p-4 text-right text-slate-400 font-mono text-xs">{area.id}</td>

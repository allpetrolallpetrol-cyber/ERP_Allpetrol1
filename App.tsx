...
const Layout = ({ children }: { children?: React.ReactNode }) => {
  // Mobile: Sidebar is hidden by default. Desktop: Sidebar is open by default.
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden relative">
      ...
      <aside 
        className={`
          fixed z-40 h-full bg-primary text-white transition-all duration-300 ease-in-out flex flex-col shadow-2xl border-r border-zinc-800
          md:relative
          ${isSidebarOpen 
            ? 'w-64 translate-x-0'  // Open: Mobile full width, Desktop full width
            : '-translate-x-full w-64 md:translate-x-0 md:w-20' // Closed: Mobile OFF SCREEN, Desktop MINI
          }
        `}
      >
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative bg-zinc-50/50">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md shadow-sm flex items-center justify-between px-4 md:px-6 z-10 border-b border-zinc-200/50 shrink-0">
          <div className="flex items-center gap-3">
              ...
              <button 
                onClick={toggleSidebar} 
                className="md:hidden text-zinc-500 p-1 hover:text-accent transition-colors"
                aria-label="Abrir menú"
              >
                  <Menu size={24} />
              </button>
              
              <h2 className="text-base md:text-lg font-semibold text-zinc-800 flex items-center truncate">
                {location.pathname === '/' && <><HomeIcon className="mr-2 text-accent hidden md:block" size={20}/> Home</>}
                {location.pathname === '/commercial' && <><ShoppingCart className="mr-2 text-accent hidden md:block" size={20}/> Gestión Comercial</>}
                {location.pathname === '/master-data' && <><Database className="mr-2 text-accent hidden md:block" size={20}/> Datos Maestros</>}
                {location.pathname === '/maintenance' && <><Wrench className="mr-2 text-accent hidden md:block" size={20}/> Mantenimiento</>}
                {location.pathname === '/warehouse' && <><Package className="mr-2 text-accent hidden md:block" size={20}/> Almacenes</>}
                {location.pathname === '/users' && <><Users className="mr-2 text-accent hidden md:block" size={20}/> Usuarios</>}
              </h2>
          </div>
...
const Home = () => (
  <div className="h-full flex flex-col items-center justify-center pb-20 animate-in fade-in duration-700">
    ...
    <h2 className="text-2xl md:text-3xl font-bold text-zinc-800 mb-4 tracking-tight text-center px-4">Bienvenido a PyME ERP</h2>
    <p className="text-zinc-500 text-base md:text-lg max-w-md text-center leading-relaxed px-4">
        Sistema integral de gestión empresarial.
        <br/>
        <span className="text-xs md:text-sm mt-2 block text-zinc-400">Seleccione una funcionalidad en el menú lateral para comenzar a operar.</span>
    </p>
